...
export const LeadsDemographic = () => {
  const [leads, setLeads] = useState<Lead[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [countryLeads, setCountryLeads] = useState<CountryLeadsData>({});
  ...
  const calculateCountryLeads = (leadsData: Lead[]) => {
    const countryCounts: CountryLeadsData = {};
    
    // Filter leads by selected course
    const filteredLeads = selectedCourse === 'All Courses' 
      ? leadsData 
      : leadsData.filter(lead => {
          const leadCourse = (lead as any).Courses || (lead as any).attributes?.Courses;
          return leadCourse === selectedCourse;
        });
    
    filteredLeads.forEach(lead => {
      const country = (lead as any).Country || (lead as any).attributes?.Country;
      if (country) {
    ...
    setCountryLeads(countryCounts);
  };

  useEffect(() => {
    if (leads.length > 0) {
      calculateCountryLeads(leads);
    }
  }, [selectedCourse, leads]);

  // Get top countries sorted by lead count
  const topCountries = Object.entries(countryLeads)
    .sort(([, a], [, b]) => b.count - a.count)
    .slice(0, 10);

  const totalLeads = Object.values(countryLeads).reduce((sum, country) => sum + country.count, 0);

  // Create markers for countries with leads - recalculate when countryLeads changes
  const markers = React.useMemo(() => {
    return topCountries
      .map(([code]) => {
        const coords = getCountryCoordinates(code);
        if (coords && countryLeads[code]) {
          return {
            latLng: coords,
            name: countryLeads[code].name,
            style: {
              fill: "#3b82f6",
              borderWidth: 2,
              borderColor: "white",
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Number of leads based on country
          </p>
        </div>
        {/* Course Filter */}
        <div className="flex items-center gap-3">
          ...
          <select
            ...
            className="px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm hover:shadow-md transition-shadow"
          >
            {courses.map((course) => (
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="h-80 w-full">
            <DynamicVectorMap
              key={`map-${selectedCourse}-${topCountries.length}`}
              map={worldMill}
          ...
          <div className="h-80 overflow-y-auto pr-2 space-y-4" style={{
            ...
            {topCountries.length > 0 ? (
              ...
              <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                <p>No leads with country information available{selectedCourse !== 'All Courses' ? ` for ${selectedCourse}` : ''}</p>
              </div>
            )}
          </div>
        </div>
