...
export const CourseStudentsChart = () => {
  ...
  const calculateCourseStudents = (): CourseData[] => {
    const allCourses = [
      "General English",
      "Level 3 Business Management",
      "Level 3 Law",
    ...
    return coursesToShow.map(course => {
      ...
      const active = courseStudents.filter(student => student.enrollmentStatus === 'Active').length;
      const completed = courseStudents.filter(student => student.enrollmentStatus === 'Completed').length;
      const suspended = courseStudents.filter(student => student.enrollmentStatus === 'Suspended').length;
      const withdrawn = courseStudents.filter(student => student.enrollmentStatus === 'Withdrawn').length;
  ...
  const courseData = calculateCourseStudents();
    ...
    <div className="rounded-xl border border-gray-200 bg-white dark:bg-gray-800 dark:border-gray-700 p-6 shadow-sm">
      ...
      <div className="mb-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-green-600 shadow-sm">
          ...
          <div>
            ...
            <div className={`grid ${getGridCols()} gap-4`}>
              {courseData.map((course) => (
                <div 
                  key={course.course} 
                  className="group bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-all duration-200 hover:border-green-300 dark:hover:border-green-600"
                >
                  <div className="p-5">
                    {/* Course Name */}
                    <div className="mb-4">
                    ...
                    <div className="space-y-2 pt-4 border-t border-gray-100 dark:border-gray-700">
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="text-gray-900 dark:text-white font-semibold">{course.active}</span>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-900 dark:text-white font-semibold">{course.completed}</span>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-900 dark:text-white font-semibold">{course.suspended}</span>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <div className="flex items-center gap-2">
                          <div className="w-1.5 h-1.5 rounded-full bg-red-500"></div>
                          <span className="text-gray-600 dark:text-gray-400 font-medium">Withdrawn</span>
                        </div>
                        <span className="text-gray-900 dark:text-white font-semibold">{course.withdrawn}</span>
                      </div>
                    </div>
                  </div>
                </div>
