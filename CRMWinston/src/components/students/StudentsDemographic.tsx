...
// Country name to ISO code mapping (reused from LeadsDemographic)
...
export const StudentsDemographic = () => {
  const [students, setStudents] = useState<Student[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [countryStudents, setCountryStudents] = useState<CountryStudentsData>({});
  ...
  const calculateCountryStudents = (studentsData: Student[]) => {
    const countryCounts: CountryStudentsData = {};
    ...
    setCountryStudents(countryCounts);
  };

  useEffect(() => {
    if (students.length > 0) {
      calculateCountryStudents(students);
    }
  }, [selectedCourse, students]);

  // Get top countries sorted by student count
  const topCountries = Object.entries(countryStudents)
    .sort(([, a], [, b]) => b.count - a.count)
    .slice(0, 10);

  const totalStudents = Object.values(countryStudents).reduce((sum, country) => sum + country.count, 0);

  // Create markers for countries with students
  const markers = React.useMemo(() => {
    return topCountries
      .map(([code]) => {
        const coords = getCountryCoordinates(code);
        if (coords && countryStudents[code]) {
          return {
            latLng: coords,
            name: countryStudents[code].name,
            style: {
              fill: "#10b981",
              borderWidth: 2,
              borderColor: "white",
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Number of students based on country
          </p>
        </div>
        {/* Course Filter */}
        <div className="flex items-center gap-3">
          ...
          <select
            ...
            className="px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm font-medium focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent shadow-sm hover:shadow-md transition-shadow"
          >
            {courses.map((course) => (
          ...
          <div className="h-80 overflow-y-auto pr-2 space-y-4" style={{
            ...
            {topCountries.length > 0 ? (
              topCountries.map(([code, data]) => {
                const percentage = totalStudents > 0 ? (data.count / totalStudents * 100).toFixed(1) : '0';
                return (
              ...
              <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                <p>No students with country information available{selectedCourse !== 'All Courses' ? ` for ${selectedCourse}` : ''}</p>
              </div>
            )}
          </div>
        </div>
